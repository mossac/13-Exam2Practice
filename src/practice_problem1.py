...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        add_contents=''
        if volume < len(additional_contents):

            for k in range(volume):
                if k <= len(additional_contents):
                    add_contents = add_contents + additional_contents[k]
            self.contents = self.contents + add_contents
            for h in range(volume, len(additional_contents)):
                end = end + additional_contents[h]
   ...
   --       #             the new volume]
    ...
    def combined_box(self, other_box):
        ...
        new_box = Box(new_contents,volume)
        return new_box
